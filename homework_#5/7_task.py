...
with open('7_task.txt', 'r') as f:
    for line in f.readlines():
        firm, form, gain, cost = line.split()
        profit = float(gain) - float(cost)
        if profit > 0:
            all_profit += profit
            profit_firms_count += 1
        firms[firm] = profit
        average['average_profit'] = all_profit / profit_firms_count
    out_list.append(firms)
    out_list.append(average)
